        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {dropdownOpen && (
            <div className="absolute top-full right-0 mt-1.5 bg-white rounded-lg shadow-lg w-[200px] z-[1000] max-h-[300px] overflow-y-auto border border-gray-200">
              <div 
                className={`p-2.5 cursor-pointer transition-colors text-gray-700 flex items-center justify-between ${selectedSprint === "all" ? 'text-blue-600 font-medium bg-blue-50' : 'hover:bg-gray-100'}`}
              ...
              {Array.isArray(sprints) && sprints.length > 0 ? sprints.map(sprint => (
                <div 
                  key={sprint.id} 
                  className={`p-2.5 cursor-pointer transition-colors text-gray-700 flex items-center justify-between ${parseInt(selectedSprint) === sprint.id ? 'text-blue-600 font-medium bg-blue-50' : 'hover:bg-gray-100'}`}
